...
export const CourseCard: React.FC<CourseCardProps> = ({
  ...
  return (
    <>
      <div
        className="flex flex-col h-full course-card cursor-pointer"
      ...
      >
        <div className="bg-white w-full group rounded-[1px] overflow-hidden flex flex-col h-full shadow-lg hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 relative">
          <div
            ref={mediaRef}
            className="relative h-60 overflow-hidden shrink-0 bg-gray-100"
          >
            <video
              ref={videoRef}
              src={offering.video}
            ...
            {(!offering.video || offering.video === "undefined") && (
              <img
                src={
                  offering.image ||
                  "https://placehold.co/600x400/002B4E/white?text=Course"
                }
                alt={offering.title}
                className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
              />
            ...
            <div className="absolute top-4 left-4 z-10">
              <span
                ref={categoryRef}
                className="items-center gap-1.5 px-3.5 py-1.5 bg-brand-gold border uppercase border-brand-gold rounded-full text-white text-xs font-bold font-sans tracking-[1px]"
              >
                {offering.category}
              </span>
            </div>
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-[#002B4E]/95 backdrop-blur-sm p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300 flex gap-2 z-20">
              <button
                ...
                className="flex-1 bg-white text-[#002B4E] hover:bg-[#C2B067] hover:text-white text-[10px] font-bold uppercase py-2.5 rounded-sm flex items-center justify-center gap-2 border border-transparent transition-colors tracking-[1px]"
              >
              ...
              <button
                onClick={(e) => {
                  ...
                  e.stopPropagation();
                  inCompare
                    ? removeFromCompare(offering.id)
                    : addToCompare(offering);
                }}
                className={`flex-1 text-[10px] font-bold uppercase py-2.5 rounded-sm flex items-center justify-center gap-2 transition-all tracking-[1px] ${
                  inCompare
          ...
          <div className="p-4 flex-1 flex flex-col relative z-10 bg-white">
            ...
            <h3
              ref={titleRef}
              className="text-lg lg:text-xl font-serif font-bold text-[#002B4E] mb-4 leading-tight group-hover:text-[#1289fe] transition-colors origin-top-left"
            >
              {offering.title}
            </h3>
            ...
            <div className="mb-4 flex justify-between items-end border-t border-gray-100 pt-3">
              <div>
                <p className="text-[10px] uppercase text-gray-400 font-bold mb-1 tracking-[1px]">
                  Tuition
                </p>
